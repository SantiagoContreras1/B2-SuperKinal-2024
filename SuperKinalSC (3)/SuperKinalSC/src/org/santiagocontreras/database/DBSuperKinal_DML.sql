...
DELIMITER $$
	create procedure sp_AgregarDetalleCompra(cantC int, proId int)
    begin
		insert into DetalleCompra(cantidadCompra,productoId) values
			(cantC,proId);
    end $$
DELIMITER ;
...
DELIMITER $$
    ...
    begin
		SELECT 
				...
				CONCAT('Producto: ',Productos.nombreProducto, ' | ', 'Descripción: ', Productos.descripcionProducto) AS ProductoDetalle
			FROM 
				DetalleCompra
			JOIN Productos ON DetalleCompra.productoId = Productos.productoId;
    end $$
DELIMITER ;
...
DELIMITER $$
create procedure sp_editarDetalleCompra(dtId int,cantC int, proId int)
	BEGIN
...
begin
    ...
    CONCAT(' | ','Nombre: ',c.nombre,' | ','Apellido: ',c.apellido,' | ','Nit: ', c.nit,' | ', 'Telefono: ',c.telefono,' | ','Direccion: ',c.direccion,' | ') AS 'Cliente',
    CONCAT(' | ','Nombre: ',e.nombreEmpleado,' | ','Apellido: ',e.apellidoEmpleado,' | ') AS 'Empleado' from Facturas f
    
    JOIN Clientes c ON f.clienteId = c.clienteId
    JOIN Empleados e ON e.empleadoId = e.empleadoId;
...
begin
	select CONCAT('ID: ',df.detalleFacturaId,' | ','ID Factura: ',df.facturaId,' | ','ID Producto',df.productoId) AS 'Detalle Factura',
    CONCAT('Producto: ',p.nombreProducto) AS 'Producto' from DetalleFactura df
...
create procedure sp_listarPromociones()
	begin
        ...
        CONCAT('ID: ',pr.productoId, ' | ','Nombre: ',pr.nombreProducto,' | ','Precio: ',pr.precioVentaUnitario) AS 'Producto'from Promociones p
...
-- ********************Agregar *****************************
DELIMITER $$
create procedure sp_AgregarUsuario(us varchar(30), pas varchar(100), nivAccId int, empId int)
begin
	insert into Usuarios(usuario,pass, nivelAccesoId, empleadoId)
    values(us,pas,nivAccId,empId);
end $$
DELIMITER ;
